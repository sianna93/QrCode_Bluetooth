...
/**
 * Created by Sianna-chan on 29/01/2017.
 */
...
public class ConexionBT {
    ...
    private synchronized void setState(int estado) {
        ...
        mHandler.obtainMessage(Bluetooth_activity.Mensaje_Estado_Cambiado, estado, -1).sendToTarget();
    }
    ...
    public synchronized void connected(BluetoothSocket socket, BluetoothDevice device) {
        ...
        Message msg = mHandler.obtainMessage(Bluetooth_activity.Mensaje_Nombre_Dispositivo);
        Bundle bundle = new Bundle();
        bundle.putString(Bluetooth_activity.DEVICE_NAME, device.getName());
        msg.setData(bundle);
    ...
    private void connectionFailed() {
        ...
        Message msg = mHandler.obtainMessage(Bluetooth_activity.Mensaje_TOAST);
        Bundle bundle = new Bundle();
        bundle.putString(Bluetooth_activity.TOAST, "Error de conexión");
        msg.setData(bundle);
        mHandler.sendMessage(msg);
    }
    ...
    private void connectionLost() {
        ...
        Message msg = mHandler.obtainMessage(Bluetooth_activity.Mensaje_TOAST);
        Bundle bundle = new Bundle();
        bundle.putString(Bluetooth_activity.TOAST, "Se perdio conexion");
        msg.setData(bundle);
        mHandler.sendMessage(msg);
        msg = mHandler.obtainMessage(Bluetooth_activity.MESSAGE_Desconectado);
        mHandler.sendMessage(msg);
...
//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        ...
        public void write(byte[] buffer) {
            try {
                ...
                mHandler.obtainMessage(Bluetooth_activity.Mensaje_Escrito, -1, -1, buffer).sendToTarget();
        ...
        public void run() {
            ...
            while (true) { //Mantiene escuchando el InputStream mientras este conectado
                try {
                    ...
                    mHandler.obtainMessage(Bluetooth_activity.Mensaje_Leido, bytes, -1, buffer).sendToTarget(); //readMessageX por buffer
